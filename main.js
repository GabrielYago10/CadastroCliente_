    ...
    try {
        ...
        if (error.code === 11000) {
            dialog.showMessageBox({
                type: 'error',
                title: "Atenção!",
                message: "CPF já cadastro.\nVerifique o número digitado."
            }).then((result) => {
                // se o botão ok for pressiondo
                if (result.response === 0) {
                    //Limpar o campo CPF
                }
            })
...
async function relatorioCliente() {
    try {
        // ==============================================================
        //                 Configuração do documento pdf
        // ==============================================================
        // p (portrati) l (landscape)
        ...
        // console.log(cliente)
        // popular o documento pdf com os clientes cadastros
        clientes.forEach((c) => {
        ...
        // =============================================================
        //     Abrir o Arquivo odf no  sistema operacional     
        // =============================================================
...
ipcMain.on('search-name', async (event, cliName) => {
    // Passo 3 e 4 (busca dos dados do cliente pelo nome)
    ...
    try {
        ...
        //teste de busca do cliente pelo nome (passo 3 a 4)
        console.log(client)
        ...
        if (client.length === 0) {
            ...
            dialog.showMessageBox({
                type: 'warning',
                title: 'Aviso',
                message: 'Cliente não cadastrada.\nDeseja cadastrar este cliente?',
                defaultId: 0,
                ...
                if (result.response === 0 ) {
                    // enviar ao rendererCliente um pedido para recortar e copiar o nome do cliente do campo de busca para o  nome (evitar que usuário digite o nome novamente) 
                    event.reply('set-name')
                } else {
                   // enviar ao rendere cliente um pedido para limpar os campos (reautilizar a api do preload 'reset-form')
                event.reply('reset-form')
                }
            ...
            // enviar ao renderizador (rendererCliente) dos dados do cliete (passo 5) Obs: não esquecer de converter para string "JASON.stringify"
